"""Generate embeddings (768-dim) for skills, courses, jobs.
...
Order: **courses → jobs → skills** so the smallest collections get real API
calls first when quota is tight.

Env:
  OPENAI_API_KEY                — required (unless EMBED_FORCE_DETERMINISTIC=1)
        ...
        if not flag[0]:
            ...
            try:
                ...
                if _is_quota_exhausted(e):
                    ...
                    log.warning("[%s] batch failed (%s); trying per-item API then deterministic", name, e)
                    for t in texts:
                        if not flag[0]:
                        ...
                        try:
                            ...
                            if _is_quota_exhausted(ee):
                                log.warning(
                                    "[%s] quota exhausted mid-row — deterministic from here on.",
                                    name,
                                )
                                flag[0] = False
                                vectors.append(deterministic_embedding(t))
                            else:
                                log.warning("[%s] single embed failed: %s; using deterministic", name, ee)
                                vectors.append(deterministic_embedding(t))

        if len(vectors) != len(batch):
            raise RuntimeError(f"vector count mismatch: {len(vectors)} vs {len(batch)}")
